    ...
    */
    virtual bool AuditSignal(const StrategyResponse &signal, RiskAuditResponse &outResponse) override
    {
      ...
      // V5.1 RESEARCH MODE BYPASS (All soft penalties default to 0 in Research mode)
       ...
       // 12. V5 Institutional Momentum Sniper Engine Check (Min Candle Body & Volume Ratio)
       double minBodyPips = m_config.GetMinCandleBodyPips();
       if(minBodyPips > 0.0)
       {
          ...
          {
             double currentBodyPips = MathAbs(closePrice - openPrice) / (_Point * 10.0);
             if(currentBodyPips < minBodyPips)
             {
                outResponse.Reason = StringFormat("V5 Momentum Sniper Blocked: M15 Candle Body (%.1f pips) < Min Required (%.1f pips)", currentBodyPips, minBodyPips);
                m_logger.Warning("[RISK] " + outResponse.Reason);
                outResponse.Approved = false;
                return false;
             }
       ...
       double minVolRatio = m_config.GetMinVolumeRatio();
       if(minVolRatio > 0.0)
       {
          long curVol = iVolume(_Symbol, PERIOD_M15, 0);
          long volSum = 0;
          for(int v = 1; v <= 20; v++) volSum += iVolume(_Symbol, PERIOD_M15, v);
          double avgVol = (double)volSum / 20.0;
          if(avgVol > 0.0)
          {
             double curVolRatio = (double)curVol / avgVol;
             if(curVolRatio < minVolRatio)
             {
                outResponse.Reason = StringFormat("V5 Momentum Sniper Blocked: Volume Ratio (%.2fx) < Min Required (%.2fx)", curVolRatio, minVolRatio);
                m_logger.Warning("[RISK] " + outResponse.Reason);
                outResponse.Approved = false;
                return false;
             }
